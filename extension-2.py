...
optparser.add_option("-p", "--PROPER_W", dest="PROPER_W", default=3.2, type="float", help="Proper Noun Weight (default=2.0)")
...
proper_spanish = defaultdict(int)
for sindex, s in enumerate(s_sents):
    for s_poss in s.split():
        if s_poss.lower() not in es_map:
            proper_spanish[s_poss] += 1
                ...
                for k, e_word in enumerate(e_list):
                 ...
                 for k, word in enumerate(e_list):
                    if not e_bit_vec[k] and word == s_word and not translated and word not in stopwords:
                        ...
                        count_benefit = 1/proper_spanish[s_word]
                        count_same += count_benefit
                        translated = True
                        e_bit_vec[k] = 1
